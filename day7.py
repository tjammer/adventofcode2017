...
def part_one(inp):
    ...
    for line in inp.split('\n'):
        has_disk = len(line.split('->')) > 1
        name = line.split(' ')[0]
        if has_disk:
            dct[name]= line.split('->')[1].strip().split(', ')

    for key in dct.keys():
        found = sum(1 for vals in dct.values() for name in vals if key == name) > 0
        if not found:
            return key
...
def part_two(inp):
    ...
    def is_disc_balanced(masslist):
        return len(set(masslist)) < 2

    def find_wrong_index(masslist):
        counts = {}
        for val in set(masslist):
            counts[val] = sum(1 for mass in masslist if mass == val)
        for i, mass in enumerate(masslist):
            if counts[mass] == 1:
                return i

    def sum_mass(name, dct):
        return weights[name] + sum(sum_mass(nm, dct) for nm in dct[name])

    candidates = []
    for name, val in dct.items():
        masslist = [sum_mass(nm, dct) for nm in dct[name]]
        if not is_disc_balanced(masslist):
            wrong_index = find_wrong_index(masslist)
            mass_difference = sum_mass(val[wrong_index], dct) - sum_mass(val[(wrong_index + 1) % len(val)], dct)
            # if multiple candidates, take the one with the lowest sum_mass
            candidates.append((val[wrong_index], sum_mass(val[wrong_index], dct), weights[val[wrong_index]] - mass_difference))

    name, sum_mass, correct_mass = sorted(candidates, key=lambda x:x[1])[0]
    return correct_mass
